...
检查当前进程的邮箱寻找匹配给定模式的消息。```receive/1```函数支持很多子句，如```case/2```。
也可以给子句加上卫兵表达式。

如果找不到匹配的消息，当前进程将一直等待，知道下一条信息到达。可以设置一个超时时间：
```
iex> receive do
...>   {:hello, msg}  -> msg
...> after
...
超时时间设为0表示你知道当前邮箱内肯定有邮件存在，很自信，设这个极短的超时时间。
